...
/**
 * @file   KimeraVIO.cpp
 * @brief  Example of VIO pipeline, dataset type is euroc or euroc like (custom) but without ground truth data
...
DEFINE_int32(dataset_type,
             ...
             "0: Euroc \n 1: Euroc custom no GT \n 2: Kitti (not supported).");
// add path to custom parameters directory
DEFINE_string(
    euroc_params_folder_path,
    "../params/Euroc",
    "Path to the folder containing the yaml files with the VIO parameters.");

DEFINE_string(
    euroc_custom_params_folder_path,
    "../params/euroc_like_noGT",
    "Path to the folder containing the yaml files with VIO parameters for euroc like format, no ground truth.");

std::string active_params_folder_path;
...
int main(int argc, char* argv[]) {
  ...
  // Conditionally set the active_params_folder_path based on dataset_type
  if (FLAGS_dataset_type == 0) {
    active_params_folder_path = FLAGS_euroc_params_folder_path;
  } else if (FLAGS_dataset_type == 1) {
    active_params_folder_path = FLAGS_euroc_custom_params_folder_path;
  } else {
    std::cerr << "Warning: Unsupported dataset_type. Using Euroc params as default." << std::endl;
    active_params_folder_path = FLAGS_euroc_params_folder_path;
  }
  VIO::VioParams vio_params(active_params_folder_path);
  ...
  switch (FLAGS_dataset_type) {
    ...
    case 1: {
      switch (vio_params.frontend_type_) {
        case VIO::FrontendType::kMonoImu: {
          dataset_parser =
              std::make_unique<VIO::MonoNoGtDataProvider>(vio_params);
        } break;
        case VIO::FrontendType::kStereoImu: {
          dataset_parser = std::make_unique<VIO::NoGtDataProvider>(vio_params);
        } break;
        default: {
          LOG(FATAL) << "Unrecognized Frontend type: "
                     << VIO::to_underlying(vio_params.frontend_type_)
    ...
    default: {
      LOG(FATAL) << "Unrecognized dataset type: " << FLAGS_dataset_type << "."
                 << " 0: EuRoC, 1: custom no GT 2: Kitti.";
    }
  }
