 ...
 */
class NoGtDataProvider : public DataProviderInterface {
 public:
  KIMERA_DELETE_COPY_CONSTRUCTORS(NoGtDataProvider);
  KIMERA_POINTER_TYPEDEFS(NoGtDataProvider);
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  //! Ctor with params.
  NoGtDataProvider(const std::string& dataset_path,
                    const int& initial_k,
                    const int& final_k,
                    const VioParams& vio_params);
  //! Ctor from gflags
  explicit NoGtDataProvider(const VioParams& vio_params);

  virtual ~NoGtDataProvider();
...
class MonoNoGtDataProvider : public NoGtDataProvider {
 public:
  KIMERA_DELETE_COPY_CONSTRUCTORS(MonoNoGtDataProvider);
  KIMERA_POINTER_TYPEDEFS(MonoNoGtDataProvider);
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  
  MonoNoGtDataProvider(const std::string& dataset_path,
                        const int& initial_k,
                        const int& final_k,
                        const VioParams& vio_params);

  explicit MonoNoGtDataProvider(const VioParams& vio_params);

  virtual ~MonoNoGtDataProvider();
